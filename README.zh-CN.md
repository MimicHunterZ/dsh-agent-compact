...
压缩通常意味着全量清扫:官方引擎只能从会话开头压,开头的任务规划、方向也会随着信息压缩丢失一部分。`context_compact` 只压缩 **agent 选中的那一段** —— 一个完成的工作步骤、一段排查完的日志、一段跑偏的讨论 —— 重要的开头和最近的内容原样保留。**区间压缩把压缩带来的信息丢失降到最小**:哪些内容真的没用了,由 agent 自己判断,只压那一部分。
...
人做事时也是这样:最近发生的事记得最牢、原样保留;中间的记忆**不会整体清空** —— 能总结的总结成要点,重要的细节原样记住。`context_compact` 让 agent 用同样的方式管理对话:

- **最近的内容** —— 原样保留,不动;
- **中段的内容** —— 不是无差别压缩:能总结的(排查完的日志、跑偏的讨论、完成的一步)巩固成检查点,重要的细节**逐字保留**在检查点里;
- **开头的方向** —— 任务规划、意图原样保留。

所以压缩不是"丢信息",而是像人一样**选择性巩固**:该忘的总结掉,该留的一个字都不少。
